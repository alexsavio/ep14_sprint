
import os
from fast_comparison import group_files_alex, group_files_borja

def test_SimpleDicomFileDistance(group_files_func):

    from fast_comparison import SimpleDicomFileDistance, DicomFilesClustering
    from os.path import abspath, dirname

    #datadir = os.path.join(abspath(dirname(__file__)), 'dicoms')
    datadir = 'dicoms'
    file1 = os.path.join(datadir, 'subj1_01.dcm')
    file2 = os.path.join(datadir, 'subj2_01.dcm')
    file3 = os.path.join(datadir, 'subj2_02.dcm')
    file4 = os.path.join(datadir, 'subj2_03.dcm')
    file5 = os.path.join(datadir, 'subj2_04.dcm')
    file6 = os.path.join(datadir, 'subj2_05.dcm')

    dist = SimpleDicomFileDistance()
    # Metemos todos los ficheros que tengamos en una 
    dcm_files = [file1, file2, file3, file4, file5, file6]

    groups = group_files_func(dcm_files)
    return groups

    #dcmclus = DicomFilesClustering(groups_leaders)
    #dcmclus._calculate_file_distances()
    #return dcmclus

if __name__ == '__main__':
    print('Alex test running...')
    groups_alex = test_SimpleDicomFileDistance(group_files_alex)

    print('Borja test running...')
    groups_borja = test_SimpleDicomFileDistance(group_files_borja)

